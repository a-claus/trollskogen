...
function kontrollKarta2()
{
	...
	besokta.push({ruta:4,heraki:heraki, status:"otestad"});
	console.log(besokta[0].status);
	...
	console.log("besokta:"+besokta.length +"/" +" otestade"+ raknadeRutor.length);
		...
		//kolla vilka rutor man kommer till
			...
			there={ruta:here.ruta, heraki:here.heraki, status:here.status};
			there=vilkenruta(there, i); //nummer och heraki
			...
			if (there.ruta>-1 && there.ruta<81){
				...
				console.log("besokt:"+besokt+" heraki:"+ heraki);
				console.log(besokta.length);
				console.log(besokta);
				...
				if (besokt>-1){		
					...
					if (there.heraki<here.heraki){ 
						...
						{
							if (besokta[k].heraki>there.heraki){
								besokta[k].heraki=there.heraki; 
								here.heraki=there.heraki;
								heraki=there.heraki;
